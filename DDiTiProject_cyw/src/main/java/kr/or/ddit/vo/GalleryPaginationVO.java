...
@Data
public class GalleryPaginationVO<T> {
	
	private int totalRecord; // 총 게시글 수
	private int totalPage; // 총 페이지 수
	private int currentPage; // 현재 페이지
	private int screenSize = 12; // 페이지 당 게시글 수
	private int mainscreenSize = 12;  // 페이지 당 게시글 수(메인에 보여지는)
	private int blockSize = 12; // 페이지 블록 수
	...
	public GalleryPaginationVO() {
	}
	
	public GalleryPaginationVO(int screenSize, int blockSize) {
		super();
	...
	public String getPagingHTML() {
		...
		for (int i = startPage; i <= (endPage < totalPage ? endPage : totalPage); i++) {
			if (i == currentPage) {
				html.append("<li class='page-item active'><a class='page-link'>" + i + "</a></li>");
			} else {
				html.append(
