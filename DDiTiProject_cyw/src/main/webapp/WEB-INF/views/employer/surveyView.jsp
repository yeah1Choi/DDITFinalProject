...
<section class="content">  
	...
	<div class="col-10">
		...
		<div class="col-12">
			...
			<c:forEach items="${surveyVO.srvQstList }" var="qst">
				<div class="box container examQst" idx="${qst.qstNo }">
					<div class="row box-header with-border" style="display: flex;">
						...
						<div class="col-1"><h4>${qst.qstNo }.</h4></div>
						<div class="col-10"><h4 class="box-title">${qst.srvQst }</h4></div>
				  ...
				  <div class="row box-footer bt-1 border-light p-10">
				  	...
				  	<c:forEach items="${qst.srvQstChList }" var="choice" >
					  	<div class="col-10 answerStr">
							<a class="media media-single qstCh p-0" href="#" idx="qst${qst.qstNo }-${choice.chNo}">
								<div class="col"><h5>(${choice.chNo}).</h5></div>
								<div class="col-10"><h5>${choice.chStr }</h5></div>
								<div class="col"></div>
							</a>
						</div>
						<div class="col-2 answer">
							<font style="text-align: center;">
								<input type="radio" name="qst${qst.qstNo }" idq="${qst.srvQstNo }" idc="${choice.chNo}"
									id="qst${qst.qstNo }-${choice.chNo}">
								<label for="qst${qst.qstNo }-${choice.chNo}" class="h-20 p-10 mb-0"></label>
							</font>				  	
					  	</div>
				  	</c:forEach>
				  </div>
			...
			<div class="text-end">
				<button type="button" id="surSubmit" name="surSubmit" class="btn btn-primary-light">제출</button>
				<c:if test="${memRole eq 'ROL003' }">
					<button id="deltBtn" name="delBtn" class="btn btn-primary-light">삭제  </button>
				</c:if>
			</div>
